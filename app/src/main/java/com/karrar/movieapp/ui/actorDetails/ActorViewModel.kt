...
import com.karrar.movieapp.ui.gallery.ActorGalleryUIState
...
class ActorViewModel @Inject constructor(
    ...
    private val getActorImagesUseCase: GetActorGalleryUseCase,
    ...
    override fun getData() {
        ...
        viewModelScope.launch {
            try {
                ...
                val actorMovies = getActorMoviesUseCase(args.id)
                    .map { actorMoviesUIMapper.map(it) }
                val socialMediaLinks =
                    actorSocialMediaUIMapper.map(getActorSocialMediaUseCase(args.id))
                val actorGallery = getActorImagesUseCase(args.id)
                val galleryImage: List<String> = actorGallery.galleryUrl
                Log.e("nour",galleryImage.toString())
                _actorDetailsUIState.update {
                    it.copy(
                        ...
                        actorGalleryUIState = ActorGalleryUIState(
    ...
    override fun onClickSeeAllGallery() {
        _actorDetailsUIEvent.update { Event(ActorDetailsUIEvent.SeeMoreGallery) }
    }
    ...
    override  fun onClickSocialMediaLink(link: String) {
        _actorDetailsUIEvent.update { Event(ActorDetailsUIEvent.OpenSocialMediaLinkEvent(link)) }
    }
}